(function(){
  ...
  exports.Expressions = (function() {
    ...
    Expressions.prototype.make_return = function make_return() {
      ...
      if (last instanceof CommentNode) {
        last = this.expressions[(idx = idx - 1)];
      }
      ...
      for (_b = 0, _c = _a.length; _b < _c; _b++) {
        ...
        if (prop.soak_node) {
          ...
          if (this.base instanceof CallNode && prop === props[0]) {
            ...
            complete = complete + this.SOAK + ((baseline = baseline + prop.compile(o)));
          }
        } else {
          part = prop.compile(o);
  ...
  exports.IfNode = (function() {
    ...
    IfNode.prototype.is_chain = function is_chain() {
      this.chain = this.chain || this.else_body && this.else_body instanceof IfNode;
      return this.chain;
    };
    // The **IfNode** only compiles into a statement if either of its bodies needs
    // to be a statement. Otherwise a ternary is safe.
    IfNode.prototype.is_statement = function is_statement() {
      this.statement = this.statement || !!(this.comment || this.tags.statement || this.body.is_statement() || (this.else_body && this.else_body.is_statement()));
