...
exports.Rewriter: class Rewriter
  ...
  close_open_calls_and_indexes: ->
    ...
    @scan_tokens (prev, token, post, i) =>
      switch token[0]
        ...
        when '('           then parens[parens.length - 1]: parens[parens.length - 1] + 1
        when '['           then brackets[brackets.length - 1]: brackets[brackets.length - 1] + 1
        when ')'
        ...
        when ']'
          if brackets[brackets.length - 1] = 0
            ...
            brackets[brackets.length - 1]: brackets[brackets.length - 1] - 1
      return 1
  ...
  add_implicit_parentheses: ->
    ...
    @scan_tokens (prev, token, post, i) =>
      tag: token[0]
      switch tag
        when 'CALL_START' then calls: calls + 1
        when 'CALL_END'   then calls: calls - 1
